...
describe('Starting tests for Foodsupply-Chain', function() {
  ...
  it('website title should be Foodsupply-Chain', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('Foodsupply-Chain');
    })
  });
  ...
  it('navbar-brand should be Foodsupply-Chain',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('Foodsupply-Chain');
    });
  });
